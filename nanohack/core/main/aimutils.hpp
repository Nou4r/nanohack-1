namespace aimutils {
	...
	Vector3 get_prediction( ) {
		...
		float bullet_speed = (itemModProjectile->GetRandomVelocity( ) * (plusminus::ui::get_bool(xorstr_("fast bullets")) ? 1.4f : 1.f)) * base_projectile->projectileVelocityScale( );

		if (base_projectile->class_name_hash() == STATIC_CRC32("CompoundBowWeapon"))
			bullet_speed = (itemModProjectile->GetRandomVelocity( ) * (plusminus::ui::get_bool(xorstr_("fast bullets")) ? 1.4f : 1.f)) * reinterpret_cast<CompoundBowWeapon*>(base_projectile)->GetProjectileVelocityScale( );
